...
24. Contact-form notifications are explicitly plain text. The source URL and submitted fields use newline separators, submitted markup is stripped while multiline values are retained, and `wp_mail()` receives a `text/plain` content-type header. This corrects the upstream experiment's mixture of HTML markup, malformed `</br>` separators, and default plain-text delivery while preserving the email-content filter.
